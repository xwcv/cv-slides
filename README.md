...
- `←` / `→` / 空格：翻页；`Home` / `End`：首末页；点击底部进度条跳转
- `O` 或 `Esc`：总览模式（缩略图带页码，点击跳转）
- `F`：全屏；`T`：暗色/浅色切换（跟随系统偏好，localStorage 记忆）；`?` 或 `H`：快捷键帮助；支持 URL `#/N` 定位与触摸滑动

## 内容

| 文件 | 讲次 | 页数 |
|---|---|---|
| `lecture1.html` | 第 1 讲 深度学习与计算机视觉简介 | 56 |
| `lecture2.html` | 第 2 讲 从 AlexNet 到智能体 | 98 |
